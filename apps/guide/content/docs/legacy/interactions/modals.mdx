...
<Callout>Radio groups require at least two options to be sent</Callout>
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		// [!code focus:12]
		// [!code ++:9]
		const radioGroupLabel = new LabelBuilder().setLabel('Radio Group').setRadioGroupComponent((radioGroup) =>
			radioGroup.setCustomId('radiogroup').addOptions([
				{ label: 'Option 1', value: 'option_1', description: 'The first option' },
				{ label: 'Option 2', value: 'option_2', description: 'The default option', default: true },
		...
		// Add components to modal
		// [!code ++:1]
		modal.addLabelComponents(radioGroupLabel);
...
A Radio Group component can be customized to apply validation via the following `RadioGroupBuilder` methods:

```js
const radioGroup = new RadioGroupBuilder()
	// Set the optional identifier for component
	.setId(1)
	// Require a option to be selected in this component (defaults to true)
	.setRequired(true);
```
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		const checkboxGroupLabel = new LabelBuilder().setLabel('Checkbox Group').setCheckboxGroupComponent((checkboxes) =>
			checkboxes.setCustomId('checkboxes').addOptions([
				{ label: 'Option A', value: 'option_a', description: 'The first option' },
				{ label: 'Option B', value: 'option_b', description: 'The default option', default: true },
...
A Checkbox Group component can be customized to apply validation via the following `CheckBoxGroupBuilder` methods:

```js
const checkboxes = new CheckboxGroupBuilder()
	// Set the optional identifier for component
	.setId(1)
	// Minimum number of boxes that must be checked (defaults to 1); min 0, max 10
	.setMinValues(1)
	// Maximum number of boxes that can be checked (defaults to 1); max 10
	.setMaxValues(1)
	// Require a box to be check in this component (defaults to true)
	.setRequired(true);
```
...
<Callout>
	Checkbox components can not be required. If a checkbox is to be required a Checkbox Group component needs to be used
	instead.
</Callout>
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		const checkboxLabel = new LabelBuilder()
			.setLabel('Did you enjoy this modal?')
			.setCheckboxComponent((checkbox) => checkbox.setCustomId('checkbox'));

		// Add components to modal
		// [!code --:1]
		modal.addLabelComponents(radioGroupLabel, checkBoxesLabel);
		// [!code ++:1]
		modal.addLabelComponents(radioGroupLabel, checkboxGroupLabel, checkboxLabel);
...
![Additional Modal Example](./images/additional-modal-example.png)

### Receiving Additional Modal Components
...
client.on(Events.InteractionCreate, (interaction) => {
	...
	if (interaction.customId === 'myModal') {
		...
		// [!code focus:4]
		// Get the data entered by the user
		const radioGroup = interaction.fields.getRadioGroup('radiogroup');
		const checkboxes = interaction.fields.getCheckboxGroup('checkboxes');
		const checkbox = interaction.fields.getCheckbox('checkbox');
